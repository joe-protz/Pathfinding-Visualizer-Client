...
const Welcome = (props) => {
  return (
    <Fragment>
      ...
      <p>The first step is choosing an algorithm, as the Begin button will not do anything until you have chosen an algorithm to visualize. There are also buttons to generate a random selection of walls or weighted cells. When you tell the animation to begin by clicking on the [Begin/Reset] button the algorithm will begin drawing a path from the top left to the bottom right corner while avoiding walls. For this demonstration, it is able to pass through diagonal walls if they do not form a corner. There may be be other colors to help you understand what is happening based on the algorithm. If there is no solution because of walls, it will stop at the best solution found.</p>
    </Fragment>
  )
}
